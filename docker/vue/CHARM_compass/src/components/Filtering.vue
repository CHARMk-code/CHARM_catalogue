      ...
      <v-expansion-panel-content>
        <v-row justify="center">
          <v-col
            ...
            max-width="48"
            >
            <p style="width: 48px;" class="text-center ma-0 pa-0"> {{ prg.name.toUpperCase() }} </p>
            <v-btn-toggle v-model="activeFilters[prg.id]">
              <v-btn
                class="pa-0"
                @click="filterCompanies(prg.id)"
                >
                <v-img
                  contain
        ...
        for (let i = 0; i < this.activeFilters.length; i++) {
          ...
          if (filter === 0) {
            ...
            filterCompanies.forEach(companyId => {
              if (!filteredCompanies.some(oldCompany => oldCompany.id === companyId)) {
                filteredCompanies.push(this.$store.getters.getCompanyById(companyId))
              }
            })
          }
        }
