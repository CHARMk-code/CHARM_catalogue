...
from ...models import Company_card, createGeneric, deleteGeneric, updateGeneric
...
def company_view_put():
    ...
    def create_or_update(one_request_data):
        ...
        if not id:
            return send_status(createGeneric(Company_card, [name,text,active]))

        company_card_object = Company_card.query.get(id)

        return updateGeneric(Company_card, company_card_object, [name,text,active])
...
def reset_company_view():
    ...
    for card in cards: 
        deleteGeneric(card)
    status = map(lambda card: createGeneric(Company_card, card), default_cards)
