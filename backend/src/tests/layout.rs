...
async fn get_by_id_when_no_matching_layout_should_fail(db: Pool<Postgres>) -> Result<(), Error> {
    // Setup
    let initial_row_amount = sqlx::query!("SELECT count(*) FROM layouts")
        .fetch_all(&db)
        .await?;
...
#[sqlx::test(fixtures("Layouts"))]
async fn valid_update_on_existing_layout_should_update_row_in_db(
    db: Pool<Postgres>,
) -> Result<(), Error> {
    // Setup
    let initial_layouts = services::layout::get_all(&db).await.unwrap();
    ...
    let updated_first_layout = updated_layouts
        .iter()
        .cloned()
        .find(|r| r.id == initial_first_layout.id)
        .unwrap();
    ...
    assert_eq!(
        initial_other_layouts, updated_other_layouts,
...
async fn delete_on_existing_id_should_remove_correct_row_in_db(
    ...
    let remaining_layout_rows = sqlx::query!("SELECT id FROM layouts")
        .fetch_all(&db)
        .await?;
    assert!(
        remaining_layout_rows.iter().all(|r| r.id != removed_id),
        "Should not return removed id when querying remaining rows"
    );
    assert_eq!(
        remaining_layout_rows.len() + 1,
        initial_layouts.len(),
