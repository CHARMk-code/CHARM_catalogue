...
async fn get_by_id_when_no_matching_map_should_fail(db: Pool<Postgres>) -> Result<(), Error>{
    // Setup
    let initial_row_amount = sqlx::query!("SELECT count(*) FROM maps").fetch_all(&db).await?;
    ...
    assert!(queried_map.is_err(), "Should fail when querying for nonexisting id");
...
async fn creating_a_valid_map_should_create_row_in_db(db: Pool<Postgres>) -> Result<(), Error> {
    ...
    let new_created_map: &services::map::MapDB = new_maps.iter()
        .filter(|r| &r.id == created_query_result.as_ref().unwrap())
        .collect::<Vec<&services::map::MapDB>>().first().unwrap();
    ...
    assert_eq!(&expected_map, new_created_map, "Making sure the map has been properly created in the database");
    assert_eq!(initial_maps.len() + 1, new_maps.len(), "One row should have been added to table");
    assert_eq!(new_other_maps, initial_maps,"Other rows should NOT have been changed");
...
async fn valid_update_on_existing_map_should_update_row_in_db(db: Pool<Postgres>) -> Result<(), Error> {
    ...
    let initial_other_maps = initial_maps.iter().filter(|r| r.id != initial_first_map.id).collect::<Vec<&services::map::MapDB>>();
    ...
    assert_eq!(update_query_result.unwrap(), initial_first_map.id, "Update should return the id of the updated row");
    
    // Check updates of map table
    let updated_maps = services::map::get_all(db.clone()).await.unwrap();
    let updated_first_map = updated_maps.iter().cloned().filter(|r| r.id == initial_first_map.id).next().unwrap();
    let updated_other_maps = updated_maps.iter().filter(|r| r.id != initial_first_map.id).collect::<Vec<&services::map::MapDB>>();
    ...
    assert_eq!(initial_other_maps, updated_other_maps, "Update should not affect other rows");
...
async fn delete_on_existing_id_should_remove_correct_row_in_db(db: Pool<Postgres>) -> Result<(), Error> {
    ...
    assert!(removed_map.is_err(), "Database query should fail for removed id");
    
    //Check that map has been removed
    let remaining_map_rows = sqlx::query!("SELECT id FROM maps").fetch_all(&db).await?;
    assert!(remaining_map_rows.iter().all(|r| r.id != removed_id.clone()), "Should not return removed id when querying remaining rows");
    assert_eq!(remaining_map_rows.len()+1, initial_maps.len(), "Remaining rows +1 should be equal to initial number of rows" );
