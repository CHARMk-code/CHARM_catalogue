...
async fn get_by_id_when_no_matching_feedback_should_fail(db: Pool<Postgres>) -> Result<(), Error>{
    // Setup
    let initial_row_amount = sqlx::query!("SELECT count(*) FROM feedback").fetch_all(&db).await?;
    ...
    assert!(queried_feedback.is_err(), "Should fail when querying for nonexisting id");
...
async fn creating_a_valid_feedback_should_create_row_in_db(db: Pool<Postgres>) -> Result<(), Error> {
    ...
    let new_created_feedback: &services::feedback::FeedbackDB = new_feedbacks.iter()
        .filter(|r| &r.id == created_query_result.as_ref().unwrap())
        .collect::<Vec<&services::feedback::FeedbackDB>>().first().unwrap();
    ...
    assert_eq!(&expected_feedback, new_created_feedback, "Making sure the feedback has been properly created in the database");
    assert_eq!(initial_feedbacks.len() + 1, new_feedbacks.len(), "One row should have been added to table");
    assert_eq!(new_other_feedbacks, initial_feedbacks,"Other rows should NOT have been changed");
...
async fn valid_update_on_existing_feedback_should_update_row_in_db(db: Pool<Postgres>) -> Result<(), Error> {
    ...
    let initial_other_feedbacks = initial_feedbacks.iter().filter(|r| r.id != initial_first_feedback.id).collect::<Vec<&services::feedback::FeedbackDB>>();
    ...
    assert_eq!(update_query_result.unwrap(), initial_first_feedback.id, "Update should return the id of the updated row");
    
    // Check updates of feedback table
    let updated_feedbacks = services::feedback::get_all(db.clone()).await.unwrap();
    let updated_first_feedback = updated_feedbacks.iter().cloned().filter(|r| r.id == initial_first_feedback.id).next().unwrap();
    let updated_other_feedbacks = updated_feedbacks.iter().filter(|r| r.id != initial_first_feedback.id).collect::<Vec<&services::feedback::FeedbackDB>>();
    ...
    assert_eq!(initial_other_feedbacks, updated_other_feedbacks, "Update should not affect other rows");
...
async fn delete_on_existing_id_should_remove_correct_row_in_db(db: Pool<Postgres>) -> Result<(), Error> {
    ...
    assert!(removed_feedback.is_err(), "Database query should fail for removed id");
    
    //Check that feedback has been removed
    let remaining_feedback_rows = sqlx::query!("SELECT id FROM feedback").fetch_all(&db).await?;
    assert!(remaining_feedback_rows.iter().all(|r| r.id != removed_id.clone()), "Should not return removed id when querying remaining rows");
    assert_eq!(remaining_feedback_rows.len()+1, initial_feedbacks.len(), "Remaining rows +1 should be equal to initial number of rows" );
