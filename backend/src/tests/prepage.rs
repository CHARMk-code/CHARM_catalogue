...
async fn get_by_id_when_no_matching_prepage_should_fail(db: Pool<Postgres>) -> Result<(), Error> {
    // Setup
    let initial_row_amount = sqlx::query!("SELECT count(*) FROM prepages")
        .fetch_all(&db)
        .await?;
...
async fn creating_a_valid_prepage_should_create_row_in_db(db: Pool<Postgres>) -> Result<(), Error> {
    ...
    assert_eq!(
        &expected_prepage, new_created_prepage,
        "Making sure the prepage has been properly created in the database"
    );
    assert_eq!(
        initial_prepages.len() + 1,
        new_prepages.len(),
        "One row should have been added to table"
    );
    assert_eq!(
        new_other_prepages, initial_prepages,
...
#[sqlx::test(fixtures("Prepages"))]
async fn valid_update_on_existing_prepage_should_update_row_in_db(
    db: Pool<Postgres>,
) -> Result<(), Error> {
    // Setup
    let initial_prepages = services::prepage::get_all(&db).await.unwrap();
    ...
    assert_eq!(
        initial_other_prepages, updated_other_prepages,
...
async fn delete_on_existing_id_should_remove_correct_row_in_db(
    ...
    let remaining_prepage_rows = sqlx::query!("SELECT id FROM prepages")
        .fetch_all(&db)
        .await?;
    assert!(
    ...
    assert_eq!(
        remaining_prepage_rows.len() + 1,
        initial_prepages.len(),
