...
async fn get_by_id_when_no_matching_tag_should_fail(db: Pool<Postgres>) -> Result<(), Error>{
    // Setup
    let initial_row_amount = sqlx::query!("SELECT count(*) FROM tags").fetch_all(&db).await?;
    ...
    assert!(queried_tag.is_err(), "Should fail when querying for nonexisting id");
...
async fn creating_a_valid_tag_should_create_row_in_db(db: Pool<Postgres>) -> Result<(), Error> {
    ...
    let new_created_tag: &services::tag::TagDB = new_tags.iter()
        .filter(|r| &r.id == created_query_result.as_ref().unwrap())
        .collect::<Vec<&services::tag::TagDB>>().first().unwrap();
    ...
    assert_eq!(&expected_tag, new_created_tag, "Making sure the tag has been properly created in the database");
    assert_eq!(initial_tags.len() + 1, new_tags.len(), "One row should have been added to table");
    assert_eq!(new_other_tags, initial_tags,"Other rows should NOT have been changed");
...
async fn valid_update_on_existing_tag_should_update_row_in_db(db: Pool<Postgres>) -> Result<(), Error> {
    ...
    let initial_other_tags = initial_tags.iter().filter(|r| r.id != initial_first_tag.id).collect::<Vec<&services::tag::TagDB>>();
    ...
    assert_eq!(update_query_result.unwrap(), initial_first_tag.id, "Update should return the id of the updated row");
    
    // Check updates of tag table
    let updated_tags = services::tag::get_all(db.clone()).await.unwrap();
    let updated_first_tag = updated_tags.iter().cloned().filter(|r| r.id == initial_first_tag.id).next().unwrap();
    let updated_other_tags = updated_tags.iter().filter(|r| r.id != initial_first_tag.id).collect::<Vec<&services::tag::TagDB>>();
    ...
    assert_eq!(initial_other_tags, updated_other_tags, "Update should not affect other rows");
...
async fn delete_on_existing_id_should_remove_correct_row_in_db(db: Pool<Postgres>) -> Result<(), Error> {
    ...
    assert!(removed_tag.is_err(), "Database query should fail for removed id");
    
    //Check that tag has been removed
    let remaining_tag_rows = sqlx::query!("SELECT id FROM tags").fetch_all(&db).await?;
    assert!(remaining_tag_rows.iter().all(|r| r.id != removed_id.clone()), "Should not return removed id when querying remaining rows");
    assert_eq!(remaining_tag_rows.len()+1, initial_tags.len(), "Remaining rows +1 should be equal to initial number of rows" );
