...
pub async fn create(db: Pool<Postgres>, data: LayoutWeb) -> Result<i32, actix_web::Error> {
    ...
    let query_result = sqlx::query!("INSERT INTO layouts (image, active, placement) VALUES ($1, $2, $3) returning id",
        image, active, placement)
...
pub async fn update(db: Pool<Postgres>, data: LayoutWeb) -> Result<i32, actix_web::Error> {
    ...
    let query_result = sqlx::query!("UPDATE layouts SET image = $1, active = $2, placement = $3 where id = $4 returning id",
        if image.is_some() {image.unwrap()} else {&layout.image},
        if active.is_some() {active.unwrap()} else {&layout.active},
        if placement.is_some() {placement.unwrap()} else {&layout.placement},
        data.id)
        .fetch_one(&db).await.map_err(MyError::SQLxError)?;
...
pub async fn delete(db: Pool<Postgres>, id: i32) -> Result<u64, actix_web::Error> {
    let query_result = sqlx::query!("DELETE FROM layouts WHERE id = $1", id).execute(&db).await.map_err(MyError::SQLxError)?;
