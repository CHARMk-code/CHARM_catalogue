...
pub async fn create(db: Pool<Postgres>, data: MapWeb) -> Result<i32, actix_web::Error> {
    ...
    let query_result = sqlx::query!("INSERT INTO maps (name, image, reference) VALUES ($1, $2, $3) returning id;",
...
pub async fn update(db: Pool<Postgres>, data: MapWeb) -> Result<i32, actix_web::Error> {
    ...
    let query_result =
        sqlx::query!("UPDATE maps SET name = $1, image = $2, reference = $3 where id = $4 returning id",
                     if name.is_some() {name.unwrap()} else {&map.name},
                     if image.is_some() {image.unwrap()} else {&map.image},
                     if reference.is_some() {reference.unwrap()} else {&map.reference},
                     data.id)
        .fetch_one(&db).await.map_err(MyError::SQLxError)?;
...
pub async fn delete(db: Pool<Postgres>, id: i32) -> Result<u64, actix_web::Error> {
    let query_result = sqlx::query!("DELETE FROM maps WHERE id = $1", id).execute(&db).await.map_err(MyError::SQLxError)?;
