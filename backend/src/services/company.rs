...
pub async fn create(db: Pool<Postgres>, data: CompanyWeb) -> Result<i32, actix_web::Error> {
    ...
    struct QueryReturn { count: Option<i64> }
    let tag_count = sqlx::query_as!(QueryReturn,"SELECT count(id) from tags where id = ANY(select * from UNNEST($1::int[]))", tags).fetch_one(&db).await.map_err(MyError::SQLxError)?;
    if usize::try_from(tag_count.count.unwrap_or(0)) != Ok(tags.len()) {
        return Err(actix_web::error::ErrorUnprocessableEntity("One or more of the received company tags are not present in database"));
    };
...
last_updated, active, charmtalk, name, description, unique_selling_point, summer_job_description, summer_job_link, summer_job_deadline, contacts, contact_email, employees_world, employees_sweden, website, talk_to_us_about, logo, map_image, booth_number)
    ...
    if tags.len() > 0 {
        ...
        (SELECT * FROM UNNEST( 
          ...
          $2::int[]))
        ", new_company_id, tags, tags.len() as i32).execute(&db).await.map_err(MyError::SQLxError)?;
    }
    ...
    let company = sqlx::query_as!(CompanyDB, "SELECT C.*, array_remove(array_agg(T.id), NULL) as tags FROM Companies as C \
    ...
    struct QueryReturn { count: Option<i64> }
    let tag_count = sqlx::query_as!(QueryReturn,"SELECT count(id) from tags where id = ANY(select * from UNNEST($1::int[]))", &new_tags).fetch_one(&db).await.map_err(MyError::SQLxError)?;
    ...
    //Remove tags
    let tags_to_remove: Vec<i32> =  current_tags_set.difference(&new_tags_set).copied().collect();
    if tags_to_remove.len() > 0 {
        sqlx::query!("DELETE FROM companies_tags where company_id = $1 \
            AND tag_id = ANY (SELECT * FROM UNNEST($2::int[])) \
        ", data.id, &tags_to_remove).execute(&db).await.map_err(MyError::SQLxError)?;
    }
   
    //Add tags
    let tags_to_add: Vec<i32> = new_tags_set.difference(&current_tags_set).copied().collect();
    if tags_to_add.len() > 0 {
        sqlx::query!("INSERT INTO companies_tags (company_id, tag_id) 
            (SELECT * FROM UNNEST( 
                array_fill($1::int, ARRAY[$3::int]),
                $2::int[]))
        ", data.id, &tags_to_add, tags_to_add.len() as i32).execute(&db).await.map_err(MyError::SQLxError)?;
    }
...
pub async fn delete(db: Pool<Postgres>, id: i32) -> Result<u64, actix_web::Error> {
    let delete_company_result = sqlx::query!("DELETE FROM companies WHERE id = $1", id).execute(&db).await.map_err(MyError::SQLxError)?;

    let _delete_companies_tags_result = sqlx::query!("DELETE FROM companies_tags WHERE company_id = $1", id).execute(&db).await.map_err(MyError::SQLxError)?;

    Ok(delete_company_result.rows_affected())
}

pub async fn get_all(db: Pool<Postgres>) ->  Result<Vec<CompanyDB>, actix_web::Error> {
    let query_result = sqlx::query_as!(CompanyDB, "\
    ...
    let mapped_result = query_result.into_iter()
        .map(|company| 
            CompanyDB{ tags: Some(company.tags.unwrap_or(Vec::new())), ..company}
        ).collect();
...
pub async fn get_by_id(db: Pool<Postgres>, id: i32) -> Result<CompanyDB, actix_web::Error> {
    ...
    Ok(CompanyDB{ tags: Some(query_result.tags.unwrap_or(Vec::new())), ..query_result})
}
