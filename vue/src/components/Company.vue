<template>
  <v-container style="margin: auto">
    <v-row>
      <v-col>
        <v-sheet style="margin: auto" max-width="890" rounded="lg">
          <v-img
            contain
            class="page"
    ...
    <v-row>
      <v-container style="margin: auto" max-width="890" rounded="lg">
        <v-col>
          <v-responsive max-width="890" style="margin: auto">
            <v-row>
              <v-col>
                <v-btn height="64" v-if="this.prev > 0">
                  <router-link
  ...
  methods: {
    getCompany(id) {
      ...
      if (typeof company === "undefined") {
        company = this.$store.dispatch("retrieveCompanies").then(() => {
